...
class IA:
    def __init__(self, api_key=None):
        ...
        self.code_col = self.db["files"]  # arquivos do usuário
    ...
    def perguntar(self, user_id, mensagem):
        if not self.client: return "IA não configurada."

        contexto = self.montar_contexto(user_id, mensagem)

        prompt = f"""
Você é um assistente especialista em LogicStart (Python em português). 
Não repita literalmente o que o usuário disse.
Use o contexto abaixo para respostas:
{contexto}
...
"""
    ...
    def corrigir(self, user_id, codigo):
        ...
        return self.perguntar(user_id, "Corrija esse código mantendo a lógica original.")

    def explicar(self, user_id, codigo):
        self.salvar_codigo(user_id, "temp", codigo)
        return self.perguntar(user_id, "Explique este código passo a passo.")

    # ================= AUTOCOMPLETE =================
    def sugerir(self, codigo):
        comandos = [
            "variavel","imprimir","se","senao","fim_se",
            "repetir","fim_repetir","funcao","fim_funcao","retorna","enquanto","fim_enquanto"
        ]
        ultima = codigo.split("\n")[-1].strip()
